...
export class NormaComponent {
  ...
  deleteSelectedNormas() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.normas = this.normas.filter(
          (val) => !this.selectedNormas.includes(val)
        );
        this.selectedNormas = null;
        this.messageService.add({
          severity: "success",
          summary: "Successful",
          detail: "Normas excluídas",
          life: 3000,
        });
      },
    });
  ...
  deleteNorma(norma: Norma) {
    this.confirmationService.confirm({
      message:
        "Você tem certeza que quer deletar a norma " + norma.codigo + "?",
      header: "Confirm",
      icon: "pi pi-exclamation-triangle",
      accept: () => {
